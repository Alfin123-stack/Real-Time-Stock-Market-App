...
/* ---------- Tambahan: tipe untuk profile2 API ---------- */
  ...
  async (query?: string): Promise<StockWithWatchlistStatus[]> => {
    try {
      ...
      if (!trimmed) {
        ...
        const profiles = await Promise.all(
          top.map(
            async (
              sym
            ): Promise<{ sym: string; profile: FinnhubProfile2 | null }> => {
              try {
        ...
        results = profiles
          ...
          .filter((x): x is FinnhubSearchResult & { __exchange?: string } =>
            Boolean(x)
          );
      } else {
      ...
      const mapped: StockWithWatchlistStatus[] = results
        .map((r) => {
          // pastikan tipe r adalah FinnhubSearchResultWithExchange
          const result = r as FinnhubSearchResultWithExchange;

          const upper = (result.symbol || "").toUpperCase();
          const name = result.description || upper;
          const exchange = result.__exchange ?? result.displaySymbol ?? "US";
